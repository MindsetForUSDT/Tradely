...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || (() => { throw new Error('ADMIN_PASSWORD is required'); })();
...
const loginLimiter = rateLimit({ windowMs: 60 * 60 * 1000, max: 5, message: { error: 'Too many attempts' } });
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        res.json({
            token,
            user: {
                id: user.id, username: user.username, is_public: user.is_public, wallet_connected: user.wallet_connected,
                first_login: user.first_login, is_admin: user.is_admin, wallet_address: user.wallet_address, wallet_type: user.wallet_type
            }
        });
    } catch { res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/auth/forgot-password', async (req, res) => {
    ...
    try {
        ...
        if (!result.rows[0].secret_question) return res.status(404).json({ error: 'No secret question' });
...
app.post('/api/auth/reset-password', async (req, res) => {
    ...
    if (!username?.trim() || !secretAnswer?.trim() || !newPassword?.trim()) return res.status(400).json({ error: 'All fields required' });
...
app.post('/api/user/change-password', authenticateToken, async (req, res) => {
    ...
    if (!currentPassword?.trim() || !newPassword?.trim()) return res.status(400).json({ error: 'All fields required' });
    if (newPassword.length < 6) return res.status(400).json({ error: 'Password too short' });
    try {
...
app.post('/api/user/wallet/disconnect', authenticateToken, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        await client.query('UPDATE users SET wallet_connected = false, wallet_address = NULL, wallet_type = NULL, is_public = false WHERE id = $1', [req.user.id]);
        await client.query('DELETE FROM trades WHERE user_id = $1', [req.user.id]);
        await client.query('COMMIT');
        res.json({ success: true });
...
app.post('/api/trades/sync', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (trades.length) {
            ...
            await client.query(`INSERT INTO trades (user_id, id, pair, volume, type, timestamp) VALUES ${placeholders}`, params);
        }
        await client.query('COMMIT');
...
app.get('/api/leaderboard', async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            SELECT u.id, u.username, u.wallet_type, COALESCE(SUM(CASE WHEN t.type = 'profit' THEN t.volume ELSE -t.volume END), 0) as total_pl,
                   COUNT(t.id) as total_trades, COALESCE(ROUND(100.0 * SUM(CASE WHEN t.type = 'profit' THEN 1 ELSE 0 END) / NULLIF(COUNT(t.id), 0), 1), 0) as win_rate
            FROM users u LEFT JOIN trades t ON u.id = t.user_id
            WHERE u.is_public = true AND u.wallet_connected = true
            GROUP BY u.id HAVING COUNT(t.id) > 0 ORDER BY total_pl DESC LIMIT $1
        `, [limit]);
...
app.get('/api/admin/users', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT u.id, u.username, u.wallet_connected, COUNT(t.id) as trades_count,
                   COALESCE(SUM(CASE WHEN t.type = 'profit' THEN t.volume ELSE -t.volume END), 0) as total_pl
            FROM users u LEFT JOIN trades t ON u.id = t.user_id WHERE u.is_admin = false GROUP BY u.id ORDER BY u.created_at DESC
        `);
        res.json(result.rows);
    } catch { res.status(500).json({ error: 'Server error' }); }
});
...
function calculatePremiumAnalytics(rows) {
    ...
    return {
        ...
        winRate: Math.round(winRate*10)/10, avgProfit: Math.round(avgProfit*100)/100, avgLoss: Math.round(avgLoss*100)/100,
        profitFactor: Math.round(profitFactor*100)/100, sharpeRatio: Math.round(sharpeRatio*100)/100,
        maxDrawdown: Math.round(maxDrawdown*100)/100, monthlyProjection: Math.round(avgReturn*30*100)/100,
        bestPair: sortedPairs[0]?.[0]||'—', worstPair: sortedPairs[sortedPairs.length-1]?.[0]||'—',
