...
const createTables = async () => {
    try {
        ...
        await pool.query(`INSERT INTO users (username, password, is_admin, first_login, wallet_connected) VALUES ('admin', $1, true, false, true) ON CONFLICT (username) DO NOTHING`, [adminPass]);
...
app.post('/api/user/wallet', authenticateToken, async (req, res) => {
    ...
    try {
        await pool.query('UPDATE users SET wallet_connected = true, wallet_address = $1, wallet_type = $2, is_public = true, first_login = false WHERE id = $3', [wallet_address, wallet_type, req.user.id]);
        const demoTrades = generateWalletTrades(wallet_address);
        if (demoTrades.length > 0) {
            const values = demoTrades.map(t => `('${t.id}', ${req.user.id}, '${t.pair}', ${t.volume}, '${t.type}', ${t.timestamp})`).join(',');
            await pool.query(`INSERT INTO trades (id, user_id, pair, volume, type, timestamp) VALUES ${values} ON CONFLICT (id) DO NOTHING`);
        }
...
app.post('/api/trades', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await pool.query('INSERT INTO trades (id, user_id, pair, volume, type, timestamp) VALUES ($1, $2, $3, $4, $5, $6)', [id, req.user.id, pair.toUpperCase(), volume, type, timestamp]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: 'Ошибка' });
    }
...
app.post('/api/trades/sync', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await pool.query('DELETE FROM trades WHERE user_id = $1', [req.user.id]);
        if (trades.length > 0) {
            const values = trades.map(t => `('${t.id}', ${req.user.id}, '${t.pair}', ${t.volume}, '${t.type}', ${t.timestamp})`).join(',');
...
app.get('/api/user/stats', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`SELECT type, SUM(volume) as total_volume, COUNT(*) as count FROM trades WHERE user_id = $1 GROUP BY type`, [req.user.id]);
        let totalPL = 0, wins = 0, totalCount = 0, maxProfit = 0, maxLoss = 0, profitSum = 0, lossSum = 0;
        result.rows.forEach(row => {
            totalCount += parseInt(row.count);
            if (row.type === 'profit') { totalPL += row.total_volume; wins += parseInt(row.count); profitSum += row.total_volume; maxProfit = Math.max(maxProfit, row.total_volume / row.count); }
            else { totalPL -= row.total_volume; lossSum += row.total_volume; maxLoss = Math.max(maxLoss, row.total_volume / row.count); }
        });
        ...
        res.json({ totalPL: Math.round(totalPL * 100) / 100, winRate: Math.round(winRate * 10) / 10, totalTrades: totalCount, wins, losses: totalCount - wins, avgProfit: Math.round(avgProfit * 100) / 100, avgLoss: Math.round(avgLoss * 100) / 100, maxProfit: Math.round(maxProfit * 100) / 100, maxLoss: Math.round(maxLoss * 100) / 100 });
    } catch (err) {
        res.status(500).json({ error: 'Ошибка' });
    }
});
...
app.get('/api/premium/analytics', authenticateToken, async (req, res) => {
    try {
        ...
        if (!user.rows[0].wallet_connected && !user.rows[0].is_admin) return res.status(403).json({ error: 'Требуется Pro статус' });
        ...
        res.json({ totalTrades: rows.length, profitTrades: profitTrades.length, lossTrades: lossTrades.length, winRate: Math.round(winRate * 10) / 10, avgProfit: Math.round(avgProfit * 100) / 100, avgLoss: Math.round(avgLoss * 100) / 100, profitFactor: Math.round(profitFactor * 100) / 100, sharpeRatio: Math.round(sharpeRatio * 100) / 100, maxDrawdown: Math.round(maxDrawdown * 100) / 100, monthlyProjection: Math.round(monthlyProjection * 100) / 100, bestPair: sortedPairs[0]?.[0] || '—', worstPair: sortedPairs[sortedPairs.length - 1]?.[0] || '—', bestDay: sortedDays[0] ? { date: sortedDays[0][0], pl: Math.round(sortedDays[0][1] * 100) / 100 } : null, worstDay: sortedDays[sortedDays.length - 1] ? { date: sortedDays[sortedDays.length - 1][0], pl: Math.round(sortedDays[sortedDays.length - 1][1] * 100) / 100 } : null });
    } catch (err) {
        res.status(500).json({ error: 'Ошибка' });
    }
});
...
app.get('/api/leaderboard', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT u.id, u.username, u.wallet_type, COALESCE(SUM(CASE WHEN t.type = 'profit' THEN t.volume ELSE -t.volume END), 0) as total_pl, COUNT(t.id) as total_trades, COALESCE(ROUND(100.0 * SUM(CASE WHEN t.type = 'profit' THEN 1 ELSE 0 END) / NULLIF(COUNT(t.id), 0), 1), 0) as win_rate FROM users u LEFT JOIN trades t ON u.id = t.user_id WHERE u.is_public = true AND u.wallet_connected = true GROUP BY u.id HAVING COUNT(t.id) > 0 ORDER BY total_pl DESC LIMIT $1`, [limit]);
        res.json(result.rows.map((row, i) => ({ rank: i + 1, username: row.username, wallet_type: row.wallet_type, totalPL: Math.round(row.total_pl * 100) / 100, totalTrades: parseInt(row.total_trades), winRate: row.win_rate })));
    } catch (err) {
        res.status(500).json({ error: 'Ошибка' });
    }
});
...
app.get('/api/admin/users', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT u.id, u.username, u.wallet_connected, u.is_public, u.created_at, COUNT(t.id) as trades_count, COALESCE(SUM(CASE WHEN t.type = 'profit' THEN t.volume ELSE -t.volume END), 0) as total_pl FROM users u LEFT JOIN trades t ON u.id = t.user_id WHERE u.is_admin = false GROUP BY u.id ORDER BY u.created_at DESC`);
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: 'Ошибка' });
    }
