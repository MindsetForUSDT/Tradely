...
// Создание таблиц с транзакцией и админом
const createTables = async () => {
    ...
    try {
        ...
        if (!ADMIN_PASSWORD) {
            console.error('❌ ADMIN_PASSWORD не задан в .env! Использую временный пароль.');
        }

        const adminPassword = ADMIN_PASSWORD || 'TradeumAdmin2024!';
        console.log(`🔐 Создаю/обновляю админа: ${ADMIN_USERNAME}`);
        ...
        if (existingAdmin.rows.length === 0) {
            ...
            console.log(`✅ Админ ${ADMIN_USERNAME} создан`);
        } else {
            await client.query(
                'UPDATE users SET password = $1, is_admin = true WHERE username = $2',
                [hashedPassword, ADMIN_USERNAME]
            );
            console.log(`✅ Пароль админа ${ADMIN_USERNAME} обновлён`);
        }
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
    ...
    try {
        console.log(`🔑 Попытка входа: ${username.trim()}`);
        ...
        if (!result.rows.length) {
            console.log(`❌ Пользователь не найден: ${username.trim()}`);
        ...
        console.log(`🔍 Проверка пароля для ${username.trim()}: ${passwordMatch ? '✅' : '❌'}`);

        if (!passwordMatch) {
            return res.status(401).json({ error: 'Invalid credentials' });
        }

        console.log(`✅ Вход выполнен: ${username.trim()} (admin: ${user.is_admin})`);
...
app.post('/api/auth/reset-password', async (req, res) => {
    ...
    if (!username?.trim() || !secretAnswer?.trim() || !newPassword?.trim()) {
    ...
    try {
        ...
        if (!result.rows.length) return res.status(404).json({ error: 'User not found' });
        if (!await bcrypt.compare(secretAnswer.toLowerCase(), result.rows[0].secret_answer)) {
