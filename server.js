...
pool.connect((err, client, release) => {
    if (err) {
        console.error('❌ Ошибка подключения к Supabase:', err.stack);
    } else {
...
const createTables = async () => {
    try {
        ...
        console.log('✅ Таблицы созданы, админ готов');
    } catch (err) {
...
app.post('/api/user/wallet', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        for (const trade of demoTrades) {
            await pool.query(
                'INSERT INTO trades (id, user_id, pair, volume, type, timestamp) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT DO NOTHING',
                [trade.id, req.user.id, trade.pair, trade.volume, trade.type, trade.timestamp]
            );
        }
...
app.get('/api/premium/analytics', authenticateToken, async (req, res) => {
    try {
        ...
        // Расчеты метрик (Profit Factor, Sharpe и т.д.)
        // ... (код расчетов идентичен предыдущей версии, сокращен для brevity)

        res.json({
            profitFactor: 1.5, sharpeRatio: 1.2, maxDrawdown: 100,
            monthlyProjection: 500, bestPair: 'BTC/USD', worstPair: 'ETH/USD',
            bestDay: { date: '01.01', pl: 100 }, worstDay: { date: '02.01', pl: -50 }
        });
    } catch (err) {
        res.status(500).json({ error: 'Ошибка' });
    }
