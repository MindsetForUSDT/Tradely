...
export function ExportPanel() {
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    ...
    else if (format === 'excel') exportToExcel(filtered, `trades_${dateStr}.xlsx`);
    else window.print();
  ...
  return (
    <Card padding="md" className="space-y-4 max-w-md">
      <h3 className="text-sm font-semibold inline-flex items-center gap-1.5">
        <Icon name="export-csv" size={18} className="text-accent-green" /> Экспорт сделок
      </h3>
      <div className="flex gap-2 flex-wrap">
        {(['all', 'month', 'week', 'custom'] as const).map((r) => (
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${dateRange === r ? 'bg-accent-green text-surface' : 'bg-surface-overlay text-text-secondary hover:text-text-primary'}`}
          >
            {r === 'all'
              ? 'Всё время'
              : r === 'month'
