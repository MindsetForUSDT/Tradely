...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="glass-card p-3 text-xs shadow-lg">
      ...
      <p className="text-text-muted">Сделок: {data.trades}</p>
    </div>
  );
}
          ...
          <BarChart data={data} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
            ...
            <YAxis
              ...
              tickFormatter={(v: number) =>
                new Intl.NumberFormat('en-US', { notation: 'compact', style: 'currency', currency: 'USD' }).format(v)
              }
            />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
            <Bar dataKey="profit" radius={[6, 6, 0, 0]} maxBarSize={48} animationBegin={0} animationDuration={1000} animationEasing="ease-out">
              {data.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.profit >= 0 ? '#00FFA3' : '#FF3B5C'} fillOpacity={0.8} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
