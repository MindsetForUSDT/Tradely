...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="glass-card p-3 text-xs shadow-lg">
      ...
      <p className={d.profit >= 0 ? 'text-accent-green' : 'text-accent-red'}>
        P&L: {formatUSD(d.profit)}
      </p>
      <p className="text-text-muted">Сделок: {d.trades}</p>
    </div>
  );
}
      ...
      <div className="h-64 md:h-72">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
            ...
            <Bar dataKey="profit" radius={[6, 6, 0, 0]} maxBarSize={48} animationDuration={1000}>
              {data.map((entry, i) => (
                <Cell key={i} fill={entry.profit >= 0 ? '#00FFA3' : '#FF3B5C'} fillOpacity={0.8} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
