...
export function WalletConnect() {
  ...
  const addWallet = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/wallets`, {
        ...
        body: JSON.stringify({ user_id: userId, address: address.trim(), chain, label: label.trim() || null }),
      });
      if (res.ok) {
  ...
  const deleteWallet = async (id: string) => {
    const token = getToken();
    if (!token) return;
    await fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/wallets?id=eq.${id}`, {
      method: 'DELETE',
      headers: { apikey: import.meta.env.VITE_SUPABASE_ANON_KEY, Authorization: `Bearer ${token}` },
    });
    fetchWallets();
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Кошельки</h1><p className="text-sm text-text-muted mt-1">Автоматический импорт сделок</p></div>
        <Button variant="primary" size="sm" onClick={() => setShowForm(!showForm)}>{showForm ? 'Отмена' : '+ Кошелёк'}</Button>
      </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
            <Card padding="md" className="mb-4 space-y-4">
              <input type="text" placeholder="Адрес кошелька" value={address} onChange={e => setAddress(e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <select value={chain} onChange={e => setChain(e.target.value as BlockchainNetwork)}
                ...
                <input type="text" placeholder="Название" value={label} onChange={e => setLabel(e.target.value)}
                  className="w-full px-4 py-2.5 bg-surface-elevated border border-surface-border rounded-xl text-sm text-white placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-accent-green/30" />
              </div>
              <Button variant="primary" onClick={addWallet} isLoading={adding} className="w-full">Добавить</Button>
            </Card>
          </motion.div>
        )}
      </AnimatePresence>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn('text-[10px] px-2 py-0.5 rounded-full bg-surface-overlay', s.color)}>{s.label}</span>
                    </div>
                    <p className="text-xs text-text-muted font-mono mt-1">{shortenAddress(w.address, 8)}</p>
                  </div>
                  <button onClick={() => deleteWallet(w.id)} className="text-text-muted hover:text-accent-red">🗑</button>
                </div>
              </Card>
            );
          })}
