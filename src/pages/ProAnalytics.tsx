...
export function ProAnalytics() {
  const { trades, pnlData, tokenVolumes, weekdayPerformance, totalVolume } = useTrades({ limit: 500, daysAgo: 90 });
  const { analytics } = useAnalytics(90);

  const winRate = analytics.length > 0
    ? (analytics.filter(a => a.realized_pnl_usd > 0).length / analytics.length * 100).toFixed(1)
    : '0';
  ...
  const worstDay = analytics.length > 0
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Лучший день', value: bestDay ? formatUSD(bestDay.realized_pnl_usd) : '—', color: 'text-accent-green' },
          { label: 'Худший день', value: worstDay ? formatUSD(worstDay.realized_pnl_usd) : '—', color: 'text-accent-red' },
        ].map(m => (
